...
def add_layer_with_omission(patches, colors, size=(0, 0),
    ...
    loc_diff = np.array(loc_diff)
    loc_start = top_left - np.array([0, size[0]])
    loc_start_circle = top_left - np.array([0, size[1]])
    ...
    for ind in range(this_num):
        ...
        if omit:
            ...
            patches.append(
                ...
                Circle(loc_start + ind * loc_diff + np.append(size[1], size[0]/2), 0.5))
        else:
...
if __name__ == '__main__':
    ...
    num_list = [3, 9, 9, 18, 18, 36, 36, 72, 72, 144, 144, 288, 288]
